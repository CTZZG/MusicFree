...
function formatWordByWordLine(
    ...
    // Add end timestamp
    const endTimeMs =
        lastEndTimeMs > lineTime * 1000
            ? lastEndTimeMs
            : lineTime * 1000 + (lineDuration || 0);
...
export default class LyricParser {
        ...
        if (translation) {
            ...
            if (transLrcItems.length === 0) {
                ...
                while (p1 < this.lrcItems.length) {
                    ...
                    while (
                        transLrcItems[p2].time <
                            lrcItem.time - TIME_MATCH_TOLERANCE &&
        ...
        if (romanization) {
            ...
            if (romaLrcItems.length === 0) {
                ...
                for (let i = 0; i < this.lrcItems.length; i++) {
                    ...
                    if (
                        !item.romanization ||
                        item.romanization.trim().length === 0 ||
                        item.hasRomanizationWordByWord
                    ) {
                        continue;
                    }
    ...
    private parseAngleBracketLine(line: string): IParsedLrcItem | null {
        ...
        const lineDuration =
            lastWord.startTime + lastWord.duration - words[0].startTime;
    ...
    private parseLyricImpl(raw: string) {
        ...
        for (const line of lines) {
            ...
            const standaloneMeta = this.parseMetaImpl(trimmedLine);
            if (
                Object.keys(standaloneMeta).length > 0 &&
                !/^\[[\d:.]+\]/.test(trimmedLine)
            ) {
                meta = {
                    ...meta,
            ...
            if (lineTimeMatch) {
                ...
                for (let i = 0; i < words.length; i++) {
                    if (i < words.length - 1) {
                        words[i].duration =
                            words[i + 1].startTime - words[i].startTime;
                    } else {
                        // 最后一个词，使用行持续时间减去相对时间
                        words[i].duration =
                            startTimeMs + durationMs - words[i].startTime;
                    }
                    // 确保持续时间不小于50ms
            ...
            if (timeMatches) {
                ...
                if (isFirstLine) {
                    ...
                    meta = {
                        ...meta,
                        ...this.parseMetaImpl(firstPart.trim()),
                    };
                    isFirstLine = false;
                }
