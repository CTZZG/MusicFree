...
const nativeLyricUtil = LyricUtil as NativeLyricDecryptor;
 ...
 */
export async function decryptQRCLyric(
    encryptedHex: string,
 ...
 * @throws Error with user-friendly Chinese message on decryption failure
 */
export async function decryptKuwoLyric(
    lrcBase64: string,
    isGetLyricx: boolean = true,
): Promise<string> {
    try {
        const startTime = Date.now();
        ...
        const decrypted = await nativeLyricUtil.decryptKuwoLyric(
            lrcBase64,
            isGetLyricx,
        );
 ...
 */
export async function autoDecryptLyric(
    lyrics: string,
    enableWordByWord: boolean = false,
): Promise<string> {
    if (!lyrics) {
