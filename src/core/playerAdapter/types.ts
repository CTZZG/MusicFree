...
export interface PlayerAdapterTrack {
    ...
    ekey?: string;
    cek?: string;
    /** Internal: this remote media URL was accepted under the explicit Android
     * mpv cleartext compatibility switch. Native mpv must still route it
     * through the bounded media proxy instead of loading it directly. */
    allowInsecureHttpPlayback?: boolean;
    /** Internal: the URL is a loopback stream generated by MusicFree itself
     * (for example the CENC proxy). Native players may consume it directly. */
