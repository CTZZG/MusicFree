`2026.7.18 v0.7.1`
1. 【播放】为 MPV 手动切歌增加串行状态门控，避免旧歌曲的切歌、自然结束和播放错误事件干扰新歌曲
2. 【播放】后端确认目标歌曲已激活后再提交界面状态，降低网络歌曲快速切换时的错歌、停播和状态不同步
3. 【播放】插件无法返回有效播放地址或后端切歌失败时，恢复原歌曲、播放位置与播放状态
4. 【播放界面】竖屏详情页支持从底部上滑切换下一曲，并扩大手势判定区域
5. 【播放界面】上滑过程增加整页位移、背景视差缩放、成功上冲和失败回弹反馈
6. 【播放界面】增加双上箭头交替明灭提示，让上滑入口和触发状态更明确
7. 【歌词】播放控件整体上移，并同步抬高歌词详情操作栏，避免收藏、歌词、字体和位置菜单与播放控件重叠
8. 【桌面歌词】新增悬浮歌词逐字高亮，真实逐字时间轴按字符推进，普通 LRC 使用行时长估算并支持开关控制
9. 【播放界面】圆形封面启用独立竖屏布局，统一封面、歌曲信息、功能菜单和播放控件的纵向安全间距，避免不同区域互相覆盖
10. 【歌词】圆形封面模式的 Mini 歌词整体下移并放大当前行，长屏增加展示空间，短屏自动收紧，减少上部拥挤和下部空白
11. 【沉浸背景】封面、模糊背景与取色结果按同一视觉代际预取并同步提交，缩短切歌过渡，改善背景下半区慢一拍的问题
12. 【测试】补充 MPV 切歌恢复、手动切歌门控、上滑判定、布局边界、沉浸动画、圆形封面长短屏几何和桌面歌词 Payload 回归测试
13. 【发布】版本保持 0.7.1，Android/iOS 构建号不变
