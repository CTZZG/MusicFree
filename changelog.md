`2026.7.28 v0.7.3`
1. 【投屏】新增 Google Cast 投屏支持，播放页顶部出现投屏按钮，可投到 Chromecast、Google TV 等设备
2. 【投屏】投屏连接后播放、暂停、切歌与播放列表操作自动路由到投屏设备，本地声音自动静音
3. 【投屏】Cast 初始化改为共享单航班并支持失败重试；仅在 Nitro 后端初始化成功后显示入口，失败不会阻塞应用启动
4. 【架构】播放器内核升级至 Nitro Player 1.5.0
5. 【稳定性】恢复 Nitro 下载管理器依赖的 WorkManager 自动初始化，避免冷启动或下载组件初始化失败
6. 【均衡器】Nitro 后端均衡器接入真实 DSP，拖动滑杆会实际改变音效
7. 【均衡器】调整为标准十段（31 Hz～16 kHz），界面频率、预设与底层 DSP 一致；MPV 后端自动隐藏入口
8. 【推荐歌单】修复相同标签 ID 但请求参数变化时列表不刷新，以及切换标签后旧分页响应污染新列表的问题
9. 【音质】音质面板展示已解析、歌曲元数据、插件声明和兼容模式状态；未预先取得链接的音质可在选择时实时解析
10. 【音质】修复快速连续切换音质或切歌时旧请求覆盖新音源、音质标签与播放链接不一致及恢复进度错位的问题
11. 【播放】Android 新增 QMC 加密音源支持，可处理 MFLAC、MGG、QTag、STag/musicex、内嵌密钥和静态 QMC，并支持 Range 拖动与并发读取
12. 【播放】音源失败改为区分无可用链接、网络、插件、无效 URL、策略拒绝、不支持加密和播放器错误，并按失败类型决定重试或备用源回退
13. 【稳定性】修复 CENC/QMC HEAD 响应的 Content-Length 处理；QMC 代理增加会话上限、空闲回收和活动请求保护，断连或读取失败会及时释放资源
14. 【插件】修复部分插件搜索、榜单、歌单以及内置网易云、LRCLIB 歌词请求失败的问题
15. 【插件】修复 crypto-js 等依赖原型继承的插件加密能力被只读 facade 破坏的问题
16. 【插件】新增「允许插件使用 HTTP」开关，并修复公网 HTTP 封面、下载和封面嵌入失败的问题
17. 【插件】诊断记录只保留最近 24 小时，新增「清除诊断记录」；测试搜索卡住后可以取消
18. 【插件】订阅、批量导入和批量更新会合并能力确认；单个插件安装异常不会中断其余项目，结果仍按输入顺序展示
19. 【隐私】插件安装错误会脱敏令牌、Cookie 和本地文件路径，并限制展示长度
20. 【本地音乐】修复升级后本地歌曲因权限变化无法播放的问题，缺少权限时会主动请求并提示
21. 【本地音乐】授权文件夹后可扫描 DSF、ASF 等系统未识别格式，跨 MediaStore/SAF 扫描不再产生重复条目
22. 【下载】修复音乐标签与封面写回时可能截断或损坏原文件的问题
23. 【下载】Android 支持探测并解密 QMC 下载，按真实的 FLAC、Ogg、MP3、M4A 或 WAV 格式保存；插件换源时不会误用旧密钥
24. 【下载】CENC/QMC 解密信息写入最终化日志，失败或中断时清理临时与备份文件并支持恢复，输出格式不一致时拒绝提交损坏文件
25. 【备份】WebDAV 密码迁移失败时在设置页提示并支持一键重试；备份导出可中途退出
26. 【备份】修复用户配置的 HTTP WebDAV 上传失败问题，HTTP 与 HTTPS 共用一致的请求与重定向处理路径
27. 【播放】修复部分音源因链接协议处理不兼容而无法播放的问题
28. 【安全】插件安装与更新继续强制 HTTPS；MusicFree 提供的插件网络 API 默认拒绝本机、内网、链路本地及保留地址
29. 【安全】能力确认弹窗明确说明插件以应用权限运行，API 收敛不是安全沙箱，请只安装可信来源的插件
30. 【工程】修复 package.json 与锁文件不一致导致 GitHub Actions 在 npm ci 阶段失败的问题
31. 【依赖】将 file-type 升级到 21.3.4、uuid 升级到 11.1.1，并为旧 Jimp 接口增加兼容补丁与真实行为门
32. 【测试】全量 113 个 Jest 套件、757 个测试及 Android JVM 10 个套件、44 个测试通过；隔离 npm ci、四个补丁重放、Android Release 构建及 API 35 冷启动验证通过
33. 【安全】当前锁文件在 npm 官方 registry 审计结果为 0 个已知漏洞
34. 【发布】版本升级至 0.7.3，Android 构建号递增（400022）

`2026.7.19 v0.7.2`
1. 【数据】并发下载自动预留唯一目标路径，避免同名任务互相覆盖
2. 【缓存】修复缓存淘汰与更新交错时误删新值或新文件的问题
3. 【歌单/插件】歌单删除和插件升级采用原子提交，失败时保留原数据与旧插件
4. 【备份】恢复报告按实际新增条目计数，重复歌曲不再被误报为新增
5. 【播放】播放器启动先完成后端锁定与就绪门控，避免冷启动时误触发错误播放器后端
6. 【播放】收敛 MPV Service 销毁重建、排队命令和异步查询时序，迟到响应不再覆盖当前歌曲状态
7. 【歌词】LyricManager 初始化幂等、订阅可释放，歌词只更新当前活动播放器后端
8. 【安全】WebDAV 与插件安装统一 HTTPS、私网、重定向和凭据校验，外部脚本安装增加确认
9. 【隐私】遥测默认关闭，只有用户明确开启才发送；异常与调试日志改为最小化输出
10. 【工程】CI 增加 TypeScript 与 High 级依赖审计门禁，锁文件完成非强制安全升级并修复 Expo CLI 嵌套解析
11. 【测试】全量 71 个 Jest 套件、441 个测试通过，并完成静态审计与 Android Nitro 三段编译回归
12. 【发布】版本升级至 0.7.2，Android/iOS 构建号同步递增（400021）
