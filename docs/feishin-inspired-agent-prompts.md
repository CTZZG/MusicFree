...
## Prompt 101：插件卡片用户变量配置摘要

状态：已完成。本次继续补齐插件中心的“清楚的状态”：声明了用户变量的插件会在卡片上直接显示配置进度或缺少的配置项，并可点按进入用户变量设置。

```text
请继续在 codex/plugin-center-mvp 分支上推进插件中心的小切片：插件卡片用户变量配置摘要。

背景：
- 插件详情和健康检查已经能看到用户变量配置情况。
- 需要 Cookie、token 或其他用户变量的插件，如果没配置，用户在列表页很难一眼发现。
- plugin-center-plan 的插件详情/配置入口目标要求用户能清楚知道插件配置状态。

范围：
- `PluginItem` 对声明了 `userVariables` 的插件显示配置摘要。
- 已全部配置时显示“用户变量：已配置 x/y”。
- 有缺失时显示“缺少配置：变量名”，不显示任何变量值。
- 点按配置摘要直接打开现有 `SetUserVariables` 面板。
- 保存变量后刷新当前插件卡片摘要。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
## Prompt 102：全局搜索插件启用禁用命令

状态：已完成。本次推进统一搜索/命令面板第二阶段的第一个可逆命令：搜索插件时可直接启用或禁用插件。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：插件启用禁用命令。

背景：
- Roadmap 第二阶段明确包含“启用/禁用插件”命令。
- 当前全局搜索能搜到插件并跳转插件管理，但仍需要进入插件页后再操作开关。
- 启用/禁用是可逆动作，适合作为命令面板雏形的第一步。

范围：
- `GlobalSearch` 匹配插件时，除“打开插件管理并定位插件”外，增加“启用插件/禁用插件”命令项。
- 命令项根据当前插件启用状态动态显示启用或禁用。
- 点击后调用现有 `PluginManager.setPluginEnabled`，并刷新当前搜索结果状态。
- 成功后显示 toast。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。

不要做：
- 不新增命令编辑器。
- 不做卸载、删除、下载等破坏性或重副作用命令。
- 不改变插件排序、用户变量或诊断存储。
- 不新增报告或导出能力。

验收：
- 全局搜索输入插件名时能看到插件定位结果和启用/禁用命令。
- 当前启用插件显示“禁用插件”，当前禁用插件显示“启用插件”。
- 点击命令后插件状态切换，toast 提示成功。
- 状态切换后当前搜索结果里的命令文案刷新。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 103：全局搜索本地歌曲下一首播放命令

状态：已完成。本次继续推进统一搜索/命令面板的轻量队列动作：搜索本地歌曲时可直接添加到下一首播放。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：本地歌曲下一首播放命令。

背景：
- 全局搜索已经能展示本地歌曲直达播放结果。
- Roadmap 第二阶段包含播放和队列类命令。
- “下一首播放”是 MusicFree 已有动作，副作用轻、可理解，适合作为命令面板继续推进的一步。

范围：
- `GlobalSearch` 命中本地歌曲时，除直接播放外，为前几条命中歌曲增加“下一首播放”命令项。
- 点击后调用现有 `TrackPlayer.addNext`。
- 沿用本地文件缺失保护，缺失文件只提示重新定位，不加入队列。
- 成功后复用现有“已添加到下一首播放”toast。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
验收：
- 全局搜索输入本地歌曲名时，能看到具体歌曲和“下一首播放”命令。
- 点击具体歌曲仍会直接播放并替换为当前匹配队列。
- 点击“下一首播放”会把该歌曲加入当前队列下一首。
- 缺失文件的命令只提示重新定位，不加入队列。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 104：全局搜索本地歌曲加入歌单命令

状态：已完成。本次继续推进统一搜索/命令面板的轻量整理动作：搜索本地歌曲时可直接打开加入歌单面板。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：本地歌曲加入歌单命令。

背景：
- 全局搜索已经能展示本地歌曲直达播放和下一首播放命令。
- Roadmap 第二阶段包含“添加到歌单”等命令能力。
- 加入歌单使用现有面板，用户仍需选择目标歌单或新建歌单，风险可控。

范围：
- `GlobalSearch` 命中本地歌曲时，为前几条命中歌曲增加“加入歌单”命令项。
- 点击后打开现有 `AddToMusicSheet` 面板。
- 沿用本地文件缺失保护，缺失文件只提示重新定位，不打开加入歌单面板。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
验收：
- 全局搜索输入本地歌曲名时，能看到“加入歌单”命令。
- 点击后打开现有加入歌单面板。
- 用户选择歌单后沿用现有加入歌单成功/失败反馈。
- 缺失文件的命令只提示重新定位，不打开加入歌单面板。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 105：插件列表配置状态筛选

状态：已完成。本次继续推进插件中心的可见状态管理：插件管理首页可以按用户变量配置状态筛选插件。

```text
请继续在 codex/plugin-center-mvp 分支上推进插件中心的小切片：插件列表配置状态筛选。

背景：
- 插件卡片已经能显示用户变量配置摘要。
- 用户如果安装了很多插件，需要快速找到“缺少配置”的插件，而不是逐个滚动查看。
- plugin-center-plan 强调插件中心要让用户知道插件为什么不可用，缺少用户变量是常见原因。

范围：
- `PluginList` 增加配置状态筛选：全部配置、缺少配置、已配置、无配置。
- 筛选与现有插件名、来源、能力筛选叠加生效。
- 只判断变量是否为空，不展示或复制变量值。
- 用户变量保存后刷新列表筛选结果。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
验收：
- 插件管理页能按“缺少配置”筛出缺变量的插件。
- 选择“已配置”只显示声明变量且都已填写的插件。
- 选择“无配置”只显示未声明用户变量的插件。
- 配置完变量后，当前筛选结果能刷新。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 106：插件列表启用状态筛选

状态：已完成。本次继续推进插件中心的列表管理能力：插件管理首页可以按启用状态筛选插件。

```text
请继续在 codex/plugin-center-mvp 分支上推进插件中心的小切片：插件列表启用状态筛选。

背景：
- 插件卡片已有启用/禁用开关。
- 插件中心计划要求用户能清楚看到插件启用状态。
- 用户安装插件多时，需要快速找出已禁用插件，排查“为什么搜不到/不能播”的问题。

范围：
- `PluginList` 增加启用状态筛选：全部状态、已启用、已禁用。
- 筛选与插件名、来源、能力、配置状态筛选叠加生效。
- 插件卡片开关变化后刷新当前列表筛选结果。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
- 插件管理页能按“已启用”筛出启用插件。
- 插件管理页能按“已禁用”筛出禁用插件。
- 在筛选状态下切换某个插件开关后，列表结果能刷新。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 107：全局搜索插件诊断直达

状态：已完成。本次继续推进统一搜索/命令面板的实用入口：搜到插件后可以直接打开该插件的诊断记录。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：插件诊断直达命令。

背景：
- 插件卡片和插件诊断页已经能定位某个插件的最近诊断。
- 用户反馈插件不可用时，常见路径是先想起插件名，再需要看最近错误。
- 统一搜索应承担“快速跳转到问题现场”的角色，而不是继续堆报告/导出。

范围：
- `GlobalSearch` 命中插件时，增加“查看插件诊断”命令项。
- 点击后打开插件设置页，并默认进入插件诊断子页，同时按该插件名筛选。
- 保留现有“打开插件管理并定位插件”和启用/禁用命令。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
- 为设置页插件模块增加轻量初始子路由参数。
...
## Prompt 108：插件列表最近诊断状态筛选

状态：已完成。本次继续推进插件中心的可见问题定位能力：插件管理首页可以按最近诊断状态筛选插件。

```text
请继续在 codex/plugin-center-mvp 分支上推进插件中心的小切片：插件列表最近诊断状态筛选。

背景：
- 插件卡片已经显示最近错误摘要。
- 用户安装插件多时，需要快速找出“最近出错”的插件，而不是逐个看卡片。
- 这个能力直接服务问题定位，比继续扩展报告/导出更有用户感知。

范围：
- `PluginList` 增加诊断状态筛选：全部诊断、有最近错误、无最近错误。
- 筛选与插件名、来源、能力、配置状态、启用状态叠加生效。
- 判断逻辑复用 `getLatestPluginDiagnosticEvent(plugin.hash, plugin.name)`，保持和插件卡片最近诊断摘要一致。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
验收：
- 插件管理页能按“有最近错误”筛出最近有诊断记录的插件。
- 插件管理页能按“无最近错误”筛出没有最近诊断记录的插件。
- 诊断状态筛选能与其他筛选条件叠加。
- 清除筛选会同时重置诊断状态筛选。
- npx tsc --noEmit 通过。
- git diff --check 通过。
...
## Prompt 109：全局搜索本地歌手和专辑直达

状态：已完成。本次继续推进统一搜索和本地资料库体验：搜索本地歌手或专辑名时，可以直接进入对应智能歌单。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：本地歌手和专辑直达。

背景：
- 智能歌单已经支持按歌手、专辑聚合本地/已知音乐。
- 用户搜索歌手或专辑时，通常想看这一组歌曲，而不是只看到零散歌曲结果。
- 这让 MusicFree 的本地音乐更像资料库，也能复用已有智能歌单详情页。

范围：
- `GlobalSearch` 使用现有 `useSmartSheetFacets("artist")` 和 `useSmartSheetFacets("album")`。
- 输入命中本地歌手名时，展示“该歌手的歌曲”入口，点击进入 `SMART_SHEET_DETAIL` 的 `artist` 类型。
- 输入命中本地专辑名时，展示“专辑”入口，点击进入 `SMART_SHEET_DETAIL` 的 `album` 类型。
- 描述显示现有歌曲数量。
- 复用现有 i18n：`smartSheet.artistTitle`、`smartSheet.albumTitle`、`home.songCount`。
...
验收：
- 全局搜索输入本地歌手名时，能看到对应歌手分组入口。
- 全局搜索输入本地专辑名时，能看到对应专辑分组入口。
- 点击入口进入对应智能歌单详情页。
- 本地歌曲直达播放、下一首播放、加入歌单命令仍保留。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 110：全局搜索插件来源歌曲直达

状态：已完成。本次继续推进统一搜索和本地资料库体验：搜索插件来源名时，可以直接进入该来源的歌曲分组。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：插件来源歌曲直达。

背景：
- 智能歌单已经支持按插件来源聚合已知音乐。
- 用户搜索某个插件/来源时，有时想管理该来源的歌曲，而不是只进入插件管理。
- 这个入口让“某插件来源歌曲”从智能歌单能力变成全局可达能力。

范围：
- 在 `smartMusicSheet` 增加只读来源 facet hook，按已知音乐的 `platform` 统计歌曲数量。
- `GlobalSearch` 使用来源 facet；输入命中插件来源名时，展示“来自 {platform}”入口。
- 点击进入 `SMART_SHEET_DETAIL` 的 `plugin-source` 类型。
- 描述显示现有歌曲数量。
- 复用现有 i18n：`smartSheet.pluginSourceTitle`、`smartSheet.pluginSources`、`home.songCount`。

不要做：
- 不改变智能歌单聚合逻辑。
- 不给没有歌曲的插件来源生成空入口。
- 不新增下载、删除、编辑等副作用命令。
- 不改变插件管理搜索结果和插件启用/禁用命令。

验收：
- 全局搜索输入已有歌曲的来源名时，能看到该来源歌曲分组入口。
- 点击入口进入对应插件来源智能歌单详情页。
- 来源分组描述显示歌曲数量。
- 插件管理入口、插件诊断入口和启用/禁用命令仍保留。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 111：全局搜索智能歌单模板直达

状态：已完成。本次继续推进统一搜索和智能歌单可达性：搜索智能歌单模板名时，可以直接进入对应智能歌单详情页。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：智能歌单模板直达。

背景：
- 智能歌单已经有最近播放、最近添加、常听、收藏、本地音乐、已下载等内置模板。
- 当前全局搜索能打开智能歌单总页，但用户搜索“最近添加”或“已下载歌曲”时，更自然的是直达对应列表。
- 这个入口无副作用，能让智能歌单从“有页面”变成“随手可达”。

范围：
- `GlobalSearch` 增加内置智能歌单模板结果。
- 支持最近播放、最近添加、常听歌曲、收藏歌曲、本地音乐、已下载歌曲。
- 点击后进入 `SMART_SHEET_DETAIL`，传入对应 `SmartSheetType`。
- 新增 `globalSearch.smartSheetDescription` 文案，同步 zh-cn/en-us/zh-tw 和 i18n 类型。
- 保留现有智能歌单总页入口。

不要做：
- 不改变智能歌单聚合逻辑。
- 不新增编辑、保存或删除命令。
- 不改变已有本地音乐、歌手/专辑/来源分组搜索结果。
- 不新增独立命令面板页面。

验收：
- 全局搜索输入“最近添加”“常听歌曲”“收藏歌曲”“已下载歌曲”等关键词时，能看到对应智能歌单入口。
- 点击入口进入对应智能歌单详情页。
- 智能歌单总页入口仍保留。
- npx tsc --noEmit 通过。
- git diff --check 通过。
...
## Prompt 112：全局搜索插件设置子页直达

状态：已完成。本次继续推进统一搜索和设置项直达能力：搜索插件订阅、排序、诊断、能力矩阵时，可以直接进入对应插件设置子页。

```text
请继续在 codex/plugin-center-mvp 分支上推进统一搜索/命令面板的小切片：插件设置子页直达。

背景：
- 插件设置已经包含订阅设置、插件排序、插件诊断、能力矩阵等子页。
- 当前全局搜索能打开插件管理首页，但用户搜索“能力矩阵”“插件诊断”时，更自然的是直达对应子页。
- 这属于 Roadmap 第二阶段的“跳转到某个设置项”，无副作用，能明显减少设置页面内跳转。

范围：
- 扩展 `setting.initialPluginSettingRoute` 类型，覆盖插件设置已有子页：
  - `/pluginsetting/list`
  - `/pluginsetting/sort`
  - `/pluginsetting/subscribe`
  - `/pluginsetting/diagnostics`
  - `/pluginsetting/capability-matrix`
- `GlobalSearch` 增加插件订阅、排序、诊断、能力矩阵直接入口。
- 点击后进入设置页并默认打开对应插件设置子页。
- 复用现有 i18n，不新增文案。
...
验收：
- 全局搜索输入“插件诊断”时，可直接进入插件诊断页。
- 全局搜索输入“能力矩阵”时，可直接进入能力矩阵页。
- 全局搜索输入“插件排序”或“订阅设置”时，可进入对应子页。
- 普通“插件管理”入口仍进入插件列表。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 113：详情页副歌词默认显示设置

状态：已完成。本次继续推进歌词体验：用户可以在歌词设置里控制播放详情页是否默认显示翻译和音译。

```text
请继续在 codex/plugin-center-mvp 分支上推进歌词体验小切片：详情页副歌词默认显示设置。

背景：
- 播放详情页歌词已经支持翻译和音译显示开关。
- 这些开关只在歌词页操作区里，不容易被用户发现。
- 歌词设置里已有桌面歌词翻译/音译开关，但缺少详情页对应设置。

范围：
- 在歌词设置页增加：
  - 详情页显示翻译。
  - 详情页显示音译。
- 复用现有 `PersistStatus` 键：
  - `lyric.showTranslation`
  - `lyric.showRomanization`
- 播放详情页操作区按钮和设置页开关共享同一状态。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
验收：
- 设置 -> 歌词里能看到“详情页显示翻译”和“详情页显示音译”。
- 打开后播放详情页和封面页小歌词会按现有逻辑显示对应副歌词。
- 在播放详情页操作区切换翻译/音译后，设置页开关状态同步。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 114：下载中心专辑和来源排序

状态：已完成。本次继续推进下载资料库化：下载中心已完成记录可以按专辑名和插件来源排序。

```text
请继续在 codex/plugin-center-mvp 分支上推进下载和本地音乐库资料库化小切片：下载中心专辑和来源排序。

背景：
- 下载中心已经支持按状态、来源、歌手、专辑、写入结果和文件状态筛选。
- 已完成歌曲列表已经支持按完成时间、歌曲名、歌手名排序。
- Roadmap 里“已下载歌曲列表支持按歌手、专辑、插件来源、下载时间”还缺专辑和来源排序。

范围：
- 扩展下载中心 `DownloadSortMode`：
  - `album`
  - `source`
- 排序菜单新增：
  - 按专辑名。
  - 按插件来源。
- 专辑排序使用专辑名、歌手名、歌曲名作为稳定排序链。
- 来源排序使用插件来源、歌手名、歌曲名作为稳定排序链。
- 同步 zh-cn/en-us/zh-tw i18n 和 i18n 类型。
...
## Prompt 115：下载中心一键清除筛选和排序

状态：已完成。本次继续推进下载资料库化：下载中心可以一键清除当前筛选和排序，快速回到默认视图。

```text
请继续在 codex/plugin-center-mvp 分支上推进下载和本地音乐库资料库化小切片：下载中心一键清除筛选和排序。

背景：
- 下载中心已经支持按状态、来源、歌手、专辑、写入结果和文件状态筛选。
- 已完成歌曲列表已经支持按完成时间、歌曲名、歌手名、专辑名和插件来源排序。
- 多个筛选和排序叠加后，用户需要一个清晰入口回到默认列表。

范围：
- 当任一列表控制处于非默认状态时，筛选栏显示“清除筛选”。
- 点击后重置：
  - 下载状态筛选。
  - 插件来源筛选。
  - 歌手筛选。
...
- 默认视图不显示“清除筛选”。
- 任一筛选或排序被改变后显示“清除筛选”。
- 点击后所有筛选回到“全部”，排序回到默认顺序。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```

## Prompt 116：本地音乐一键清除筛选和排序

状态：已完成。本次继续推进本地音乐资料库化：本地音乐页可以一键清除当前筛选和排序，快速回到默认视图。

```text
请继续在 codex/plugin-center-mvp 分支上推进下载和本地音乐库资料库化小切片：本地音乐一键清除筛选和排序。

背景：
- 本地音乐页已经支持按来源、歌手、专辑和文件状态筛选。
- 本地音乐页已经支持按歌曲名、歌手名、专辑名和来源临时排序。
- 多个筛选和排序叠加后，用户需要一个清晰入口回到完整列表。

范围：
- 当任一列表控制处于非默认状态时，本地音乐筛选栏显示“清除筛选”。
- 点击后重置：
  - 来源筛选。
  - 歌手筛选。
  - 专辑筛选。
...
## Prompt 117：播放诊断补充恢复状态

状态：已完成。本次继续推进播放状态与问题诊断页：关于页播放诊断补充恢复状态，便于排查退出后进度或队列恢复问题。

```text
请继续在 codex/plugin-center-mvp 分支上推进播放诊断小切片：播放诊断补充恢复状态。

背景：
- 关于页已经有播放诊断入口，可复制当前播放、队列、后端、Native 服务和 MediaSession 摘要。
- 最近真机问题集中在退出后恢复进度、恢复歌曲和通知/服务生命周期。
- 诊断里还缺“持久化状态”和“本次启动恢复状态”的对照。

范围：
- 播放诊断快照新增 recovery 字段：
  - 持久化歌曲。
  - 持久化进度。
  - 进度保存时间。
  - 最近内存保存进度和时间。
  ...
  - 本次启动恢复进度。
  - 本次启动恢复队列长度。
- 保存 `music.progress` 时同步保存 `music.progressSavedAt`。
- 关于页“播放诊断”文本新增“恢复状态”段落。
- 同步 TrackPlayer 公开类型。
...
验收：
- 关于页播放诊断里能看到“恢复状态”段落。
- 播放一段时间后诊断可显示持久化进度和保存时间。
- 重启后诊断可显示本次启动恢复歌曲、恢复进度和恢复队列长度。
- npx tsc --noEmit 通过。
- git diff --check 通过。
```
...
当前已经完成 Prompt 01 到 Prompt 117。智能歌单 MVP 的内置模板已基本补齐，近期应继续优先做用户能直接感知的功能，例如歌词体验升级、下载和本地音乐库资料库化，或继续增强统一搜索；暂缓继续扩展测试报告和导出能力。
