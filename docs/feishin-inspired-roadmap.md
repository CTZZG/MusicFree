...
状态：已确认并开始实施。当前实现分支为 `codex/plugin-center-mvp`，插件中心已推进到本地插件重新选择文件更新、能力矩阵、插件列表来源/能力/配置/启用/最近诊断状态筛选小切片，下载和本地音乐库资料库化已补充文件状态筛选、摘要、歌手/专辑筛选、本地音乐排序、下载中心和本地音乐一键清除筛选/排序、缺失文件报告、重扫复用缺失旧记录、扫描结果可见报告和扫描结果复制能力，以及本地文件重新定位前的不匹配确认；备份恢复报告已补充恢复方式上下文，恢复前预览已支持复制；播放诊断已补充恢复状态；智能歌单已补齐“最近添加”“收藏歌曲”、歌手/专辑分组、插件来源分组、常听歌曲、临时排序、显示数量上限和保存为普通歌单；歌词体验已补充候选预览、本地音译上传和偏移实时预览；统一搜索已支持本地音乐匹配、高频页面直达、智能歌单模板直达、本地歌手/专辑/插件来源分组直达、本地歌曲直达播放、在线歌曲/歌单/专辑/歌手搜索入口、插件启用/禁用命令和插件诊断直达。
...
- 关于页播放诊断可显示构建信息、当前播放、队列索引、进度、后端状态、Native 服务和 MediaSession 摘要。
- 播放诊断可显示恢复状态，包括持久化歌曲、持久化进度、进度保存时间、本次启动恢复歌曲、恢复进度和恢复队列长度。
