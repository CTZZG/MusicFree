...
状态：已确认并开始实施。当前实现分支为 `codex/plugin-center-mvp`，插件中心已推进到本地插件重新选择文件更新、能力矩阵、插件列表来源/能力/配置/启用/最近诊断状态筛选小切片，下载和本地音乐库资料库化已补充文件状态筛选、摘要、歌手/专辑筛选、本地音乐排序、下载中心和本地音乐一键清除筛选/排序、缺失文件报告、重扫复用缺失旧记录、扫描结果可见报告和扫描结果复制能力，以及本地文件重新定位前的不匹配确认；备份恢复报告已补充恢复方式上下文，恢复前预览已支持复制；智能歌单已补齐“最近添加”“收藏歌曲”、歌手/专辑分组、插件来源分组、常听歌曲、临时排序、显示数量上限和保存为普通歌单；歌词体验已补充候选预览、本地音译上传和偏移实时预览；统一搜索已支持本地音乐匹配、高频页面直达、智能歌单模板直达、本地歌手/专辑/插件来源分组直达、本地歌曲直达播放、在线歌曲/歌单/专辑/歌手搜索入口、插件启用/禁用命令和插件诊断直达。
...
- 智能歌单详情页支持临时限制显示全部、前 25、前 50 或前 100 首，保存快照使用当前显示结果。
...
- 歌词偏移面板支持实时预览、取消恢复，以及 ±0.2s/±1s 快捷调整。
- 歌词设置页支持控制播放详情页是否默认显示翻译和音译。
...
- 下载中心已完成记录可按完成时间、歌曲名、歌手名、专辑名和插件来源排序。
- 下载中心可一键清除当前筛选和排序，快速回到默认视图。
- 下载完成记录详情、复制和导出。
...
- 重新定位文件前会读取新文件的文件名/基础标签。
- 如果新文件标题或歌手与原歌曲明显不一致，先弹出确认，避免误选文件。
- 确认后只更新 `localPath`，保留原歌曲 `id/platform/title/artist` 和普通歌单引用。
...
- 可搜索并直达最近播放、最近添加、常听、收藏、本地音乐、已下载等智能歌单模板。
- 可搜索并直达本地歌手、专辑智能歌单分组。
- 可搜索并直达插件来源智能歌单分组。
- 可搜索并直达插件订阅、排序、诊断和能力矩阵等插件设置子页。
- 本地音乐命中时展示前几条歌曲，点击可直接按当前匹配结果作为队列播放。
- 本地音乐命中时可直接把歌曲添加到下一首播放，缺失文件会被拦截提示。
- 本地音乐命中时可直接打开加入歌单面板，继续复用现有歌单选择/新建流程。
- 在线搜索区可直接选择搜索单曲、歌单、专辑或歌手。
- 搜索插件时可直接执行启用/禁用插件命令，作为命令面板雏形的第一个可逆动作。
- 搜索插件时可直接打开该插件的诊断页，并自动按插件名筛选。
