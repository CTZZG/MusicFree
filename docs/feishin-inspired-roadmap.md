...
状态：已确认并开始实施。当前实现分支为 `codex/plugin-center-mvp`，插件中心已推进到本地插件重新选择文件更新、能力矩阵和插件列表来源/能力筛选小切片，下载和本地音乐库资料库化已补充文件状态筛选、摘要、歌手/专辑筛选、本地音乐排序、缺失文件报告、重扫复用缺失旧记录、扫描结果可见报告和扫描结果复制能力，以及本地文件重新定位前的不匹配确认；备份恢复报告已补充恢复方式上下文，恢复前预览已支持复制；智能歌单已补齐“最近添加”“收藏歌曲”、歌手/专辑分组、常听歌曲、临时排序、显示数量上限和保存为普通歌单；歌词体验已补充候选预览、本地音译上传和偏移实时预览；统一搜索已支持本地音乐匹配、高频页面直达、本地歌曲直达播放、在线歌曲/歌单/专辑/歌手搜索入口、插件启用/禁用命令和插件诊断直达。
